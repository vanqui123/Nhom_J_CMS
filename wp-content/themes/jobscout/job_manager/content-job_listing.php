...
<article <?php job_listing_class(); ?> data-longitude="<?php echo esc_attr($post->geolocation_lat); ?>" data-latitude="<?php echo esc_attr($post->geolocation_long); ?>">
    ...
    color: gray;
    font-weight: 600;">Created:</span> <time style="font-size: 15px;
    color: gray;
    font-weight: 600;" datetime="<?php echo get_the_date('c'); ?>" itemprop="datePublished"><?php echo get_the_date(); ?></time> </div>
		
	<div style="display:flex ;border: 1px solid #ededed;background: #ededed;border-radius: 5px;padding: 5px;">
		<?php if ($company_name) { ?>
			<div class="company-name" style="padding-right: 5px;">
				<?php the_company_name(); ?>
			</div>
		<?php } ?>
		<?php if ($company_name) { ?>
			<div class="company-name" style="    border-left: 1px solid #afabab;
    ...
    padding: 0px 5px;">
		...
		<?php } ?>
			...
			<div class="company-address" style="padding-left: 5px;margin-top:3px">
